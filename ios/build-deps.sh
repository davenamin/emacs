...
## that typedefs the real ones fails to compile.  (Observed in CI
## as "cannot combine with previous 'type-name'" in _uid_t.h.)
## Autoconf 2.70+ turned the macro into a compile check, which is
## why Emacs's own recursive iOS configure is unaffected.  Seed
## the cache var for every dependency whose configure predates
## that fix.
fetch "$NETTLE_URL"
